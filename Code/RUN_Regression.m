function RUN_Regression( InputFile, OutputFile, SiteIndex )
...
if ~exist( [ OutputFile, '.mat' ], 'file' )
    ...
    Residual    = zeros( SiteList{ SiteIndex, 4 }, VoxelNumber );
    
    Measurement = load( InputFile );
    Temp        = fieldnames( Measurement );
    Measurement = Measurement.( Temp{ 1 } );
    
    Age = Participant_Update.Age( SiteList{ SiteIndex, 5 }:SiteList{ SiteIndex, 6 } );
    Gender = double( Participant_Update.Gender( SiteList{ SiteIndex, 5 }:SiteList{ SiteIndex, 6 } ) == 'Male' );
    Gender = 2*Gender - 1;
